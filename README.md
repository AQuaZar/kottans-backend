...
<details>
  <summary> ⌨️ Practical task  ⌨️ </summary>
...
**Schema (MySQL v8.0)**
    ...
    INSERT INTO `orders`(`person_id`) VALUES 
    	(1),
        (2),
        (4),
        (4);  
...
**Query #1**
    ...
    select first_name, last_name,count(distinct orders.id) as total_orders,sum(quantity) as total_items_bought,sum(quantity*(price-discount)) as total_money_spent from person
    left join orders on person.id = orders.person_id
...
[View on DB Fiddle](https://www.db-fiddle.com/f/xceugd67tNjdx7PSHVr4qw/51)
</details>
